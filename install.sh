...
Tools (pick one or more):
  x          x_search.ts      — search public posts on X (Twitter) via xAI
...
for arg in "$@"; do
  case "$arg" in
    x|exa|parallel) SELECTED+=("$arg") ;;
    all) SELECTED=(x exa parallel) ;;
...
echo
echo "Docs: docs/x.md, docs/exa.md, docs/parallel.md"
